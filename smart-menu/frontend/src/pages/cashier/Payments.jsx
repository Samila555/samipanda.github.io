    ...
    <div>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500">{payments.length} total submissions</p>
        </div>
        <div className="flex gap-3">
          <form onSubmit={handleSearch} className="flex gap-2">
            ...
            <button type="submit" className="bg-emerald-500 hover:bg-emerald-600 text-white font-semibold py-2 px-4 rounded-lg text-sm transition-all">Search</button>
          </form>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {payments.map((p) => (
                <tr key={p._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  ...
                  <td className="p-4 text-emerald-500 font-medium text-sm">{p.note || '-'}</td>
                  ...
                  <td className="p-4 text-center">
                    {p.screenshot ? (
                      <button onClick={() => setViewImage(p.screenshot)} className="text-emerald-500 hover:underline text-sm flex items-center gap-1 justify-center">
                  ...
                  <td className="p-4 text-center">
                    {p.paymentStatus === 'pending' && (
                      <div className="flex justify-center gap-2">
                        <button onClick={() => updateStatus(p._id, 'verified')} className="px-3 py-1.5 bg-green-500 hover:bg-green-600 text-white text-xs font-semibold rounded-lg transition-all flex items-center gap-1">
                          <CheckCircle className="w-4 h-4" /> Verify
                        </button>
                        <button onClick={() => updateStatus(p._id, 'rejected')} className="px-3 py-1.5 bg-red-500 hover:bg-red-600 text-white text-xs font-semibold rounded-lg transition-all flex items-center gap-1">
      ...
      {viewImage && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setViewImage(null)}>
          <div className="max-w-lg w-full" onClick={(e) => e.stopPropagation()}>
            ...
            <button onClick={() => setViewImage(null)} className="mt-4 mx-auto block px-6 py-2 bg-emerald-500 hover:bg-emerald-600 text-white font-semibold rounded-lg transition-all">Close</button>
          </div>
        </div>
      )}
    </div>
